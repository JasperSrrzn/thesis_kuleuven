...
def rerank_actions(actions,predicates,embeddings_enta,embeddings_entb,embeddings_rel,embeddings_rule,model,neighbors,rand_dict):
    ...
    if len(actions_to_rerank)>1:
        ...
        predictions = model.predict_proba('./classification_models/model_gcn_500/',Data)
...
embeddings_enta = pd.read_csv('entity_embeddingsa_gcn_500.csv')
embeddings_entb = pd.read_csv('entity_embeddingsb_gcn_500.csv')
embeddings_rel = pd.read_csv('relation_embeddings_gcn_500.csv')
embeddings_rule = pd.read_csv('rule_embeddings_gcn_500.csv')
...
test_data = pd.read_csv('test_data_gcn_500.csv')
...
for test_sample in range(1860,len(test_data)):
        ...
        try:
            ...
            while actions:
                ...
                if len(list(trigger_nodes))>0:
                    ...
                    pd_results.to_csv('/content/gdrive/My Drive/results_gcn_500.csv',index=False)
                    break
