...
def rerank_actions(actions,predicates,embeddings_enta,embeddings_entb,embeddings_rel,embeddings_rule,model,neighbors,rand_dict):
    ...
    if len(actions_to_rerank)>1:
        ...
        predictions = model.predict_proba('./classification_models/model_transe_200/',Data)
...
embeddings_enta = pd.read_csv('entity_embeddingsa_transe_200.csv')
embeddings_entb = pd.read_csv('entity_embeddingsb_transe_200.csv')
embeddings_rel = pd.read_csv('relation_embeddings_transe_200.csv')
embeddings_rule = pd.read_csv('rule_embeddings_transe_200.csv')
...
pd_results = pd.read_csv('results_transe_200_all.csv')
results = list(pd_results['0'])
#results = []
no_rerank_results = list(no_rerank_results['no rerank'])
for test_sample in range(1521,len(test_data)):
    try:
        print('---------------')
        ...
        while actions:
            ...
            if len(list(trigger_nodes))>0:
                ...
                pd_results.to_csv('/content/gdrive/My Drive/results_transe_200_all.csv',index=False)
                break
